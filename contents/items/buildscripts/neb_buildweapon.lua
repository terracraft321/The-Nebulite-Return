...
function build(directory, config, parameters, level, seed)
  ...
  setupAbility(config, parameters, "bonus", builderConfig, seed)
  ...
  config.tooltipFields.damagePerShotLabel = util.round(baseDps / (fireTime*2) * config.damageLevelMultiplier, 1)
  ...
  if config.bonusAbility then
    config.tooltipFields.bonusAbilityTitleLabel = "Skill:"
    config.tooltipFields.bonusAbilityLabel = config.bonusAbility.name or "unknown"
  end
