...
function init()
  ...
  self.sentMessage_hp75 = false
  self.sentMessage_hp50 = false
  self.sentMessage_hp25 = false
  self.sentMessage_hp5 = false
  self.sentMessage_hp0 = false
...
-- to catch errors
function update(dt)capturable.update(dt)
  ...
  if self.behaviorTick >= self.behaviorTickRate then
    ...
    if self.sentMessage == false then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_announce")
        self.sentMessage = true
      end
    end
    
    -- 75% HP message
    if self.sentMessage_hp75 == false and status.resource("health") <= 75 then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_hp75")
        self.sentMessage_hp75 = true
      end
    end
    
    -- 50% HP message
    if self.sentMessage_hp50 == false and status.resource("health") <= 50 then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_hp50")
        self.sentMessage_hp50 = true
      end
    end
    
    -- 25% HP message
    if self.sentMessage_hp25 == false and status.resource("health") <= 25 then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_hp25")
        self.sentMessage_hp25 = true
      end
    end
    
    -- 5% HP message
    if self.sentMessage_hp5 == false and status.resource("health") <= 5 then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_hp5")
        self.sentMessage_hp5 = true
      end
    end
    
    -- 0% HP message
    if self.sentMessage_hp0 == false and status.resource("health") <= 0 then
      local nearPlayers = world.entityQuery(entity.position(), 50, {includedTypes = {"player"}})
      nearPlayers = util.filter(nearPlayers, entity.entityInSight)
      for _,playerId in pairs(nearPlayers) do
        world.sendEntityMessage(playerId, "queueRadioMessage", "neb_raid1_hp0")
        self.sentMessage_hp0 = true
      end
    end
...
function setDamageSources()local partSources = {}
