...
function update(dt)
  if self.sentMessage == false then
    local nearPlayers = world.entityQuery(object.position(), 50, {includedTypes = {"player"}})
    for _,playerId in pairs(nearPlayers) do
      world.sendEntityMessage(playerId, "queueRadioMessage", "neb_returnconsole")
      world.sendEntityMessage(playerId, "queueRadioMessage", "neb_returnconsole1")
      world.sendEntityMessage(playerId, "queueRadioMessage", "neb_returnconsole2")
